...
/**
 * Adds a button to the Extensions dropdown menu for the bundled Moonlit Echoes variant.
 * This function creates a menu item in SillyTavern's Extensions dropdown
 * that opens the theme settings drawer when clicked.
 */
function addExtensionMenuButton() {
    ...
    // Open the inline drawer inside SillyBunny's Extensions shell instead of trapping the user in a popout.
    $button.click(() => {
        openSettingsDrawerInExtensions();
    });
}

function openSettingsDrawerInExtensions() {
    const openDrawer = () => {
        ...
        if (!(drawer instanceof HTMLElement) || !(drawerHeader instanceof HTMLElement) || !(drawerContent instanceof HTMLElement)) {
            return;
        }

        if (getComputedStyle(drawerContent).display === 'none') {
            drawerHeader.click();
        }
    ...
    window.requestAnimationFrame(() => window.requestAnimationFrame(openDrawer));
}
...
function renderExtensionSettings() {
    ...
    enabledCheckboxText.textContent = `Enable ${EXTENSION_NAME}`;
...
versionContainer.innerHTML = `
    <small class="flex-container justifyCenter alignitemscenter">
        <span>${EXTENSION_NAME} Version</span>
...
function initChatDisplaySwitcher() {
    ...
    function hasStyleOption(value) {
        const normalizedValue = String(value);
        return Array.from(chatDisplaySelect.options).some(option => option.value === normalizedValue);
    }
    ...
    const preferredChatStyle = savedChatStyle
        ?? chatDisplaySelect.dataset.sbCurrentValue
        ?? (Number.isInteger(power_user?.chat_display) && power_user.chat_display >= 0 ? String(power_user.chat_display) : null);

    if (savedTheme) themeSelect.value = savedTheme;
    if (preferredChatStyle && hasStyleOption(preferredChatStyle)) {
        chatDisplaySelect.value = preferredChatStyle;
    }
