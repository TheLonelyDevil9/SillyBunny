        ...
        eventSource.on(event_types.OAI_PRESET_CHANGED_AFTER, () => {
            ...
            document.querySelectorAll('.nemo-tray-open').forEach(section => {
                if (section._nemoCategoryTray) {
                    cleanupTrayOverlay(section._nemoCategoryTray);
...
function getSectionId(section) {
    const summary = section.querySelector('summary');
    const headerPrompt = summary?.querySelector('li.completion_prompt_manager_prompt');
    if (headerPrompt?.dataset?.nemoSectionName) {
        return headerPrompt.dataset.nemoSectionName.trim() || 'unknown';
    }
    ...
    return String(clone.textContent || '').trim() || 'unknown';
}

/**
 * Get the viewport bounds we should keep a tray overlay inside.
 * Prefer the prompt manager popup bounds so the tray stays visually attached to it.
 * @param {HTMLElement} section
 * @returns {DOMRect | { left: number, top: number, right: number, bottom: number, width: number, height: number }}
 */
function getTrayOverlayViewport(section) {
    const popup = section.closest('#completion_prompt_manager_popup');
    if (popup) {
        return popup.getBoundingClientRect();
    }

    const drawer = section.closest('.drawer-content');
    if (drawer) {
...
/**
 * Get scroll containers that can affect tray positioning.
 * @param {HTMLElement} section
 * @returns {Array<EventTarget>}
 */
function getTrayOverlayScrollTargets(section) {
    const targets = new Set([window]);
    [
        section.closest('#completion_prompt_manager_list'),
        section.closest('.completion_prompt_manager_list_pane'),
        section.closest('.completion_prompt_manager_editor_host'),
        section.closest('#completion_prompt_manager_popup'),
        section.closest('.drawer-content')
    ].filter(Boolean).forEach(target => targets.add(target));
    return Array.from(targets);
}

/**
 * Remove any floating tray listeners/RAF state.
 * @param {HTMLElement | undefined} tray
 */
function cleanupTrayOverlay(tray) {
    if (!tray) return;

    if (typeof tray._overlayCleanup === 'function') {
        tray._overlayCleanup();
        delete tray._overlayCleanup;
    }

    if (tray._overlayFrame) {
        cancelAnimationFrame(tray._overlayFrame);
        delete tray._overlayFrame;
    }

    delete tray._overlayReposition;
}

/**
 * Reposition a floating tray so it stays attached to its section header.
 * @param {HTMLElement} section
 * @param {HTMLElement} tray
 */
function positionTrayOverlay(section, tray) {
    if (!section?.isConnected || !tray?.isConnected) {
        return;
    }

    const anchor = section.querySelector('summary') || section;
    const anchorRect = anchor.getBoundingClientRect();
    const viewport = getTrayOverlayViewport(section);
    const gutter = 10;
    const viewportWidth = Math.max(0, viewport.width);
    const viewportHeight = Math.max(0, viewport.height);
    const maxWidth = Math.max(280, Math.min(760, viewportWidth - (gutter * 2)));
    const minWidth = Math.min(320, maxWidth);
    const preferredWidth = Math.max(minWidth, Math.min(maxWidth, Math.max(anchorRect.width, Math.min(440, maxWidth))));

    tray.style.width = `${Math.round(preferredWidth)}px`;
    tray.style.maxHeight = `${Math.round(Math.max(140, viewportHeight - (gutter * 2)))}px`;

    const trayRect = tray.getBoundingClientRect();
    const desiredHeight = Math.max(140, Math.min(tray.scrollHeight || trayRect.height || 0, viewportHeight - (gutter * 2)));
    const spaceBelow = Math.max(0, viewport.bottom - anchorRect.bottom - gutter);
    const spaceAbove = Math.max(0, anchorRect.top - viewport.top - gutter);
    const shouldOpenAbove = spaceBelow < Math.min(desiredHeight, 260) && spaceAbove > spaceBelow;
    const availableHeight = Math.max(140, shouldOpenAbove ? spaceAbove : spaceBelow);

    tray.style.maxHeight = `${Math.round(availableHeight)}px`;

    const measuredHeight = Math.max(160, Math.min(tray.scrollHeight || trayRect.height || desiredHeight, availableHeight));
    let left = anchorRect.left;
    let top = shouldOpenAbove
        ? anchorRect.top - measuredHeight - 6
        : anchorRect.bottom + 6;

    left = Math.min(
        Math.max(left, viewport.left + gutter),
        Math.max(viewport.left + gutter, viewport.right - preferredWidth - gutter),
    );
    top = Math.min(
        Math.max(top, viewport.top + gutter),
        Math.max(viewport.top + gutter, viewport.bottom - measuredHeight - gutter),
    );
...
/**
 * Schedule a floating tray reposition on the next animation frame.
 * @param {HTMLElement | undefined} tray
 */
function scheduleTrayOverlayPosition(tray) {
    if (!tray || typeof tray._overlayReposition !== 'function') {
        return;
    }
    ...
    tray._overlayFrame = requestAnimationFrame(() => {
        tray._overlayFrame = null;
        tray._overlayReposition();
    });
}

/**
 * Mount a tray as a floating overlay.
 * @param {HTMLElement} section
 * @param {HTMLElement} tray
 */
function mountTrayOverlay(section, tray) {
    cleanupTrayOverlay(tray);

    tray.classList.add('nemo-category-tray-floating');
    document.body.appendChild(tray);

    const scrollTargets = getTrayOverlayScrollTargets(section);
    const reposition = () => positionTrayOverlay(section, tray);

    tray._overlayReposition = reposition;
    tray._overlayCleanup = () => {
        scrollTargets.forEach(target => target.removeEventListener('scroll', reposition));
        window.removeEventListener('resize', reposition);
    };

    scrollTargets.forEach(target => target.addEventListener('scroll', reposition, { passive: true }));
    window.addEventListener('resize', reposition);

    positionTrayOverlay(section, tray);
    scheduleTrayOverlayPosition(tray);
}
...
function openTray(section) {
    ...
    // Tray mode is meant to behave like a floating overlay, not an inline expansion.
    mountTrayOverlay(section, tray);
    ...
    console.log('[NemoTray] Tray overlay mounted for section:', sectionId, tray);
